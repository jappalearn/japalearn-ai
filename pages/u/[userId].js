...
export default function PublicProfile() {
  ...
  useEffect(() => {
    ...
    // Store referral in localStorage so it carries through to signup
    localStorage.setItem('referral_from', userId)

    const load = async () => {
      const [{ data: profile }, { data: quiz }] = await Promise.all([
        supabase.from('profiles').select('full_name, avatar_url').eq('id', userId).maybeSingle(),
        supabase.from('quiz_results').select('score, answers, destination, segment').eq('user_id', userId).order('created_at', { ascending: false }).limit(1).maybeSingle(),
      ])

      if (!profile && !quiz) { setNotFound(true); setLoading(false); return }
      ...
      setData({ profile, quiz, answers, score, breakdown, insights, nextSteps, firstName, initials })
      setLoading(false)
    }
    load()
